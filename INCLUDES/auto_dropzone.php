...
/* Auto_dropzone.php v1.2 (version spéciale pour le regen=true)
    author: Bronco
...
// if there was a previous call of the script get the saved init
$phpini=ini_get_all();
if (isset($_SESSION['auto_dropzone'])){
    $auto_dropzone=$_SESSION['auto_dropzone'];
}else{
    // first call -> complete init
    if (empty($auto_dropzone['allowed_filetypes'])){            $auto_dropzone['allowed_filetypes']='png,gif,jpg,jpeg';}
    if (!isset($auto_dropzone['use_style'])){                   $auto_dropzone['use_style']=true;}// put false if you're using an external css file
    if (!isset($auto_dropzone['auto_refresh_after_upload'])){   $auto_dropzone['auto_refresh_after_upload']=true;}// auto refresh page after uploading files
    if (empty($auto_dropzone['max_length'])){                   $auto_dropzone['max_length']=5;}// Mo 
...
?>
        ...
        <div class="<?php echo $auto_dropzone['dropzone_class']; ?> DD_dropzone" id="<?php echo $auto_dropzone['dropzone_id'];?>" <?php echo $auto_dropzone['dropzone_style'];?>>
            <p class="DD_text"><?php echo $auto_dropzone['dropzone_text'];?><br/><small>(max:<?php echo $max;?> Mo)</small></p>
